...
def ngrams_model(X_train,X_test, y_train, y_test, n_range = (2, 3), top_words = 20):
    ...
    ngrams_vect = CountVectorizer(min_df = 5, ngram_range = n_range).fit(X_train['reviews'].values.astype('U'))
    X_train_ngrams_vect = ngrams_vect.transform(X_train['reviews'].values.astype('U'))
    ngrams_model.model = LogisticRegression(max_iter = 10000).fit(X_train_ngrams_vect, y_train)
    ngrams_pred = ngrams_model.predict(ngrams_vect.transform(X_test['reviews'].values.astype('U')))
    ngrams_model.accuracy = roc_auc_score(y_test, ngrams_pred)
    feature_names_ngrams = np.array(ngrams_vect.get_feature_names())
    sorted_coef_index = ngrams_model.coef_[0].argsort()
